...
**Last updated: 2026-08-27, session 7.** This file is the authoritative, reboot-surviving
...
session 5's as `.bak-session5`, session 6's as `.bak-session6`.
...
### RANK 1 — cheap, root-free, worth doing

- **>>> THE NEXT REBOOT IS REQUIRED, AND IT IS THE OUTSTANDING TEST. <<<** As this is
  written the Speaker PCM is **wedged** (leaked substream, § 4), so there is no speaker sink
  and no way to make one without rebooting. Everything else is config on disk and untested
  across a boot. After the next reboot, in order:

  ```bash
  systemctl --user is-active pipewire wireplumber pipewire-pulse   # all three active
  wpctl status | sed -n '/Sinks:/,/Sources:/p'                     # expect "Laptop Speakers"
  journalctl --user -u pipewire -b 0 | grep omnibook_spk           # empty == the node opened
  amixer -c 1 contents | grep -A2 "Right Spk Switch"               # expect values=on
  aplay -D hw:1,2 <a sweep of >=5 s>                               # NOT short bursts (§ 4)
  ```

  **If "Laptop Speakers" is missing**, the likely cause is PipeWire winning the race against
  card 1 (`pipewire.service` started at monotonic 20.88 on the 14:04 boot; card 1 registered
  at 22.65 — 1.8 s later). `10-omnibook-wait-for-card.conf` exists to prevent exactly that and
  is **unverified across a boot**. The workaround is
  `systemctl --user restart pipewire wireplumber`; the fix is that drop-in.
  **If the pin switches read `off`**, `omnibook-speaker-mixer.service` lost its race — check
  `systemctl --user status omnibook-speaker-mixer.service`.
- **Understand why ACP builds no HiFi profile** (§ 6). `pactl list cards` offers only `off`
  and `pro-audio` for this card, with a valid UCM tree that `alsaucm -c hw:1 list
  _devices/HiFi` reports 7 devices in, including Speaker, and with `ALSA_CONFIG_UCM2` verified
  present in both the `pipewire` and `wireplumber` process environments. Until this is
  understood the UCM overlay is doing nothing for the speaker path.
...
### RANK 3 — get the right-hand speakers back

The only route to stereo. Both right-side amps (device 2 on each link) die 3.2 ms before
FW_READY on every boot, with no driver bound and nothing in the window. Nothing short of
understanding the mechanism will recover them, so this is downstream of RANK 2. Do **not**
try to route around it in userspace: there is no right-side amplifier to route to.
...
### 6.1 The UCM overlay — installed, corrected, and currently inert

**Status, session 7: the overlay builds, parses, and `alsaucm -c hw:1 list _devices/HiFi`
...
profile sits at `off`. So none of the UCM work below currently reaches the speakers. It is
left enabled because it is harmless and correct, and because ACP is the thing to fix. **That
ACP failure is unexplained and is RANK 1 in § 5.**
